...
> God node · 9 connections · [/Users/mlp/Projekte/marketmind/marketmind/server/services/scraper/index.ts](file:///Users/mlp/Projekte/marketmind/marketmind/server/services/scraper/index.ts#L53)

## Call Trace Diagram

```mermaid
sequenceDiagram
    participant P0 as scrapeEbay()
    participant P1 as fetchWithConfig()
    participant P2 as scrapeKleinanzeigen()
    ...
    participant P7 as invalidateCachedHtml()
    participant P8 as buildKleinanzeigenSearchUrl()
    participant P9 as parseKleinanzeigenHtml()
    ...
    participant P24 as buildEbaySearchUrl()
    participant P25 as parseEbayHtml()
    P0->>+ P1: calls
    P1-->>- P0: return
    P1->>+ P0: calls
    P0-->>- P1: return
...
- [[buildEbaySearchUrl()]] `INFERRED`
- [[invalidateCachedHtml()]] `INFERRED`
- [[parseEbayHtml()]] `INFERRED`
