...
| **DatabaseLifecycle**       | `server/database/lifecycle.ts`                    | Relocate, Reset, Pfad-Info (`pathLocked` wenn `MM_DATABASE_PATH` gesetzt)                                                      |
| **DatabasePaths**           | `server/database/paths.ts`                        | `resolveDbPath`, `getActivePath`, `isDatabasePathLocked()` — Env überschreibt UI-Pfad (Docker)                                 |
| **MARKETMIND_DATA_DIR**     | Repo-Root `.env` / `docker-compose.yml`           | Host-Ordner für SQLite unter Docker (Bind-Mount → `/app/data/marketmind.db`)                                                   |
